...
@section('title', 'Kelola Nama Penandatangan')

@section('content')
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <!-- Form Tambah Nama Penandatangan -->
    <div class="bg-white rounded-xl shadow-sm border border-border-default p-6 h-fit">
        <h5 class="font-bold text-heading mb-4 flex items-center gap-2">
            <i class="fa-solid fa-plus-circle text-brand"></i> Tambah Penandatangan
        </h5>
        ...
        <form action="{{ route('admin.signatory.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-semibold text-heading mb-2">Nama Lengkap &amp; Gelar <span class="text-fg-danger">*</span></label>
                <input type="text" name="name" required placeholder="Contoh: Dr. H. Herri Swantoro, S.H., M.H." 
            ...
            <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-2.5 rounded-full text-sm font-bold bg-brand text-white shadow-sm hover:shadow-md hover:bg-brand-strong transition-all">
                <i class="fa-solid fa-save mr-2"></i> Simpan Penandatangan
            </button>
        </form>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-border-default overflow-hidden">
            <div class="p-6 border-b border-border-default bg-neutral-secondary-soft">
                <h5 class="font-bold text-heading m-0 flex items-center gap-2">
                    <i class="fa-solid fa-signature text-brand"></i> Daftar Nama Penandatangan
                </h5>
            </div>
                ...
                <table class="w-full text-left whitespace-nowrap text-sm text-body">
                    <thead class="bg-neutral-secondary-soft/50 border-b border-border-default text-body font-medium">
                        <tr>
                            ...
                            <th class="px-6 py-3 font-medium">Nama Penandatangan</th>
                            <th class="px-6 py-3 font-medium text-center w-[20%]">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-border-default bg-white">
                        @forelse($signatories as $index => $signer)
                            <tr class="hover:bg-neutral-secondary-soft/35 transition-colors">
                                <td class="px-6 py-4">{{ $signatories->firstItem() + $index }}</td>
                                <td class="px-6 py-4 font-semibold text-heading">{{ $signer->name }}</td>
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('admin.signatory.destroy', $signer->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus nama penandatangan ini?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center justify-center w-8 h-8 rounded-base bg-neutral-primary-soft text-danger hover:bg-danger hover:text-white border border-border-default transition-all shadow-sm" title="Hapus Penandatangan">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-8 text-center text-body-subtle italic">Belum ada data nama penandatangan terdaftar.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

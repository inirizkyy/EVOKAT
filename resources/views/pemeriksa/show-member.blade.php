...
@section('title', 'Detail Buku Registrasi Advokat - Pemeriksa')

@section('actions')
<a href="{{ route('pemeriksa.dashboard') }}" class="inline-flex items-center px-4 py-2 rounded-base text-[14px] font-medium bg-neutral-primary-soft text-brand shadow-sm hover:shadow-md hover:text-brand-strong active:shadow-inset transition-all border border-brand">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-border-default p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                ...
                <div>
                    <span class="block text-body-subtle font-medium">Pejabat Pengadilan Tinggi yang Menyumpah</span>
                ...
                <div>
                    <span class="block text-body-subtle font-medium">Status Pemeriksa</span>
                    @if($reg->status_pemeriksa === 'Disetujui')
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-success-soft border border-border-success-subtle text-fg-success-strong mt-1">
                            <i class="fa-solid fa-circle-check mr-1.5"></i>Disetujui
                        </span>
                    @else
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-warning-soft border border-border-warning-subtle text-fg-warning mt-1">
            ...
            @else
            <div class="text-body-subtle italic text-center py-4">Data BAS belum dilengkapi oleh Administrator.</div>
            @endif
        </div>
    </div>
</div>
